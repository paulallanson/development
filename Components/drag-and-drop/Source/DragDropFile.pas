...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        s := Files[i];
        {$IFDEF VER35_PLUS}AnsiStrings.{$ENDIF}StrPLCopy(p, AnsiString(s), Size);
...
begin
  ...
  {$IFDEF VER35_PLUS}AnsiStrings.{$ENDIF}StrLCopy(PAnsiChar(s), @FileGroupDescriptor^.fgd[Index].cFileName[0], MAX_PATH);
  Result := PAnsiChar(s);
end;
{$IFDEF R_PLUS}
  {$RANGECHECKS ON}
...
begin
  if (Index >= Count) then
    raise Exception.CreateFmt('Filename index out of bounds (%d)', [Index]);
  {$IFDEF VER35_PLUS}AnsiStrings.{$ENDIF}StrPLCopy(@FileGroupDescriptor^.fgd[Index].cFileName[0], Value, MAX_PATH);
end;
{$IFDEF R_PLUS}
  {$RANGECHECKS ON}
  {$UNDEF R_PLUS}
        ...
        if (AMedium.hGlobal <> 0) then
          try
            try
              ...
              try
                {$IFDEF VER35_PLUS}
                OleCheck(Stream.Seek(0, STREAM_SEEK_SET, PLargeUInt(nil)^));
                {$ELSE}
