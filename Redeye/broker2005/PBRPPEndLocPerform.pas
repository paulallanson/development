...
var
  PBRPPEndLocPerformfrm: TPBRPPEndLocPerformfrm;

implementation

{$R *.DFM}

uses PBRSPEndLocPerform;

function TPBRPPEndLocPerformfrm.GetDetails(Sender: TObject): integer;
begin
  if bConsolidate then
    begin
      with qryRevenueLocs do
        begin
          close;
          sql.Clear;
    ...
    begin
      with qryRevenueLocs do
        begin
          close;
          sql.Clear;
          sql.Text := qrytmpRevCentres.SQL.Text;
          parambyname('Invoice_Location').asinteger := SelRepCode;
          parambyname('Start').asinteger := StartPeriod;
          parambyname('Finish').asinteger := FinishPeriod;
          open;
          result := recordcount;
...
function TPBRPPEndLocPerformfrm.GetSalesDetails(Sender: TObject): integer;
begin
  with qrySalesProfit do
    begin
      close;
      if TotbyCat then
        sql.Text := qrySalesProfitCat.sql.Text
      else
...
procedure TPBRPPEndLocPerformfrm.quickreportBeforePrint(
...
procedure TPBRPPEndLocPerformfrm.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Location := qryRevenueLocs.fieldbyname('Invoice_Location').asinteger;
end;

procedure TPBRPPEndLocPerformfrm.QRSubDetail1BeforePrint(
      ...
      with qryReport do
        begin
          ...
          if TotbyCat then
            ...
            sql.Text := qryDummyNullCust.SQL.Text;
          parambyname('Category').asinteger := Category;
          parambyname('Start').asinteger := StartPeriod;
          parambyname('Finish').asinteger := FinishPeriod;
          open;
    ...
    begin
      with qryReport do
        begin
          ...
          parambyname('Location').asinteger := Location;
          parambyname('Category').asinteger := Category;
          parambyname('Start').asinteger := StartPeriod;
          parambyname('Finish').asinteger := FinishPeriod;
          open;
...
procedure TPBRPPEndLocPerformfrm.qrsubPeriodsBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  tempPeriod: integer;
begin
...
procedure TPBRPPEndLocPerformfrm.qrgRepsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
begin
...
procedure TPBRPPEndLocPerformfrm.qrbGrpCatFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
...
procedure TPBRPPEndLocPerformfrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndLocPerformfrm.qrbGrpRepFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
// Get the budgets for this rep
  with qryRevLocBudgets do
    begin
      close;
      parambyname('Invoice_Location').asinteger := Location;
      parambyname('Start_Period').asinteger := StartPeriod;
      parambyname('Finish_Period').asinteger := FinishPeriod;
      open;
      first;
...
procedure TPBRPPEndLocPerformfrm.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndLocPerformfrm.ExportToFile(fileName: string);
var
  tempStr: string;
  i: integer;
  tmpLabel: string;
...
// Build the list of periods for this year;
  ...
  tempstr := '"Revenue Centre"' + tmpLabel;
  for i := 0 to 12 do
    begin
...
procedure TPBRPPEndLocPerformfrm.qrbGrpCatFooterAfterPrint(
...
begin
  ...
  begin
    //Revenue Centre
    tempStr := '"' + qryRevenueLocs.fieldbyname('Invoice_Location_Descr').asstring + '"';
    ...
    if TotbyCust then
      tempstr := tempstr + ',"' + GetCurrentRep(qryCategory.fieldbyname('Category').asinteger) + '"';

    writeln(self.exportFile, tempStr);
    PBRSPEndLocPerformfrm.prgbrExport.StepIt;
  end;
end;

procedure TPBRPPEndLocPerformfrm.qrbGrpRepFooterAfterPrint(
  Sender: TQRCustomBand; BandPrinted: Boolean);
var
  tempStr: string;
  templbl: TQRLabel;
...
begin
  ...
  begin
    //Revenue Centre
    tempStr := '"' + qryRevenueLocs.fieldbyname('Invoice_Location_Descr').asstring + ' Totals"';
...
procedure TPBRPPEndLocPerformfrm.QRBand3AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
var
  tempStr: string;
  templbl: TQRLabel;
...
procedure TPBRPPEndLocPerformfrm.FormCreate(Sender: TObject);
begin
  TotbyCust := false;
  TotbyCat := false;
end;

function TPBRPPEndLocPerformfrm.GetCurrentRep(tempCode: integer): string;
begin
  with qryCustRep do
    begin
      close;
...
procedure TPBRPPEndLocPerformfrm.SetBudgetType(const Value: integer);
begin
  FBudgetType := Value;
end;
