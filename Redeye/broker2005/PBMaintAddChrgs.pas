...
var
  PBMaintAddChrgsFrm: TPBMaintAddChrgsFrm;
...
procedure TPBMaintAddChrgsFrm.FormActivate(Sender: TObject);
begin
  if sFuncMode = 'A' then
...
function TPBMaintAddChrgsFrm.getNextAddChrg: integer;
begin
...
procedure TPBMaintAddChrgsFrm.OKBitBtnClick(Sender: TObject);
begin
  if sFuncMode = 'A' then
  begin
...
procedure TPBMaintAddChrgsFrm.edtCostKeyPress(Sender: TObject;
  var Key: Char);
begin
  case Ord(key) of
    $30..$39: ;
...
procedure TPBMaintAddChrgsFrm.edtChargeKeyPress(Sender: TObject;
  var Key: Char);
begin
  case Ord(key) of
    $30..$39: ;
...
procedure TPBMaintAddChrgsFrm.memCostEnter(Sender: TObject);
begin
  sOldValue := (Sender as TMemo).Text;
end;

procedure TPBMaintAddChrgsFrm.SaveValue(Sender: TObject);
begin
  sOldValue := (Sender as TMemo).Text;
end;

procedure TPBMaintAddChrgsFrm.ValidateMoney(Sender: TObject);
var
  TempStr: string;
begin
...
procedure TPBMaintAddChrgsFrm.ValidateKeyPress(Sender: TObject;
  var Key: Char);
var
  P: Integer;
begin
...
procedure TPBMaintAddChrgsFrm.CheckOK(Sender: TObject);
begin
