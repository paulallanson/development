...
type
  TPBSendPOViaFTPFrm = class(TForm)
    Panel1: TPanel;
    Panel6: TPanel;
    edtUserName: TEdit;
    edtPassword: TEdit;
...
var
  PBSendPOViaFTPFrm: TPBSendPOViaFTPFrm;
...
procedure TPBSendPOViaFTPFrm.SetPurchaseOrder(const Value: TPurchaseOrder);
begin
  FPurchaseOrder := Value;
end;

procedure TPBSendPOViaFTPFrm.SetSTPODM(const Value: TSTPODM);
begin
  FSTPODM := Value;
end;

procedure TPBSendPOViaFTPFrm.btnSendClick(Sender: TObject);
var
...
function TPBSendPOViaFTPFrm.ProcessAcknowledgement: Boolean;
var
...
procedure TPBSendPOViaFTPFrm.FormActivate(Sender: TObject);
begin
...
procedure TPBSendPOViaFTPFrm.FTPStatus(ASender: TObject;
...
procedure TPBSendPOViaFTPFrm.Log(Msg: String; Color: TColor);
begin
  lstbxStatus.Items.AddObject(Msg, Pointer(Color));
  lstbxStatus.ItemIndex := lstbxStatus.Items.Count -1;
end;

procedure TPBSendPOViaFTPFrm.btnCloseClick(Sender: TObject);
begin
  close;
end;
