...
begin
  ...
  With GetContactSQL do
        ...
        First ;
        If not EOF then
                ContactQRLabel.Caption := 'For Attn: ' + FieldByName('Name').AsString 
        else
    ...
    begin
      ...
      if recordcount > 0 then
        begin
          ...
          QRLabel2.Caption := 'Picking information for '+PBPODM.GetPickCallOffSQL.FieldByName('Stock_Location_Desc').AsString;
        end
      else
        begin
...
procedure TPBRPDelivFrm.FormClose(Sender: TObject;
...
procedure TPBRPDelivFrm.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
QRLabel13.Caption := PBPODM.GetPickCallOffSQL.FieldByName('Description').AsString;
QRLabel14.Caption := formatfloat('####',PBPODM.GetPickCallOffSql.fieldByName('quantity_Picked').asfloat);
end;
