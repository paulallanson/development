...
uses
  ...
  ExtCtrls, Spin, taoCntrr, taoFrmts, ShellAPI, IniFiles, DB, ADODB, ActiveX,
...
procedure TPBMaintQuoteFrm.taoWinControl1SetDataPaste(Sender: TObject;
  Data: IInterface);
begin
  MyWinControlSetData(Data);
end;

procedure TPBMaintQuoteFrm.taoWinControl1SetDataTarget(Sender: TObject;
  Data: IInterface; X, Y: Integer);
begin
...
procedure TPBMaintQuoteFrm.taoWinControl1UpdateAction(Sender: TObject;
  Action: TtaoUpdateAction; var Enable: Boolean);
begin
...
procedure TPBMaintQuoteFrm.MyWinControlSetData(const Data: IUnknown);
const
...
begin
  if Supports(Data, ItaoCells, MyData) then
    begin
      MyPath := dmBroker.GetCompanyQuoteDirectory + '\' + floattostr(Quote.dbkey) +'\';
...
(*            QuoteDoc := TQuoteDoc.Create(Quote);
              QuoteDoc.QDocumentNo := (Quote.Documents.count + 1);
              QuoteDoc.DocumentTitle := Copy(MyFileName, 1, Length(MyFileName)-4);
              QuoteDoc.DocumentPath := Copy(MyFilePath, length(dmBroker.GetCompanyQuoteDirectory)+1,255);
              QuoteDoc.DocumentDate := date;
              QuoteDoc.Parent.Documents.Add(QuoteDoc);

*)
              ShowDocuments(Quote.dbkey);
//        sgdocuments.Row := Quote.Documents.count;
            end
          else
            begin
