...
type
  TSTMntPrtBinFrm = class(TForm)
    ...
    procedure ProductionbtnClick(Sender: TObject);
  private
...
var
  STMntPrtBinFrm: TSTMntPrtBinFrm;
...
procedure TSTMntPrtBinFrm.FormActivate(Sender: TObject);
begin
...
procedure TSTMntPrtBinFrm.LoadBinsList(Sender: TObject);
Var
TempLI: TListItem ;
begin
...
procedure TSTMntPrtBinFrm.CallUpdScreen(sTempFunc: String);
begin
STMntPrtBinUpdFrm := TSTMntPrtBinUpdFrm.Create(self) ;
Try
   STMntPrtBinUpdFrm.sFuncMode := sTempFunc ;
   STMntPrtBinUpdFrm.iStore := iStore ;
   STMntPrtBinUpdFrm.ShowModal ;
   If STMntPrtBinUpdFrm.ModalResult = mrOK then
      LoadBinsList(Self) ;
Finally
       STMntPrtBinUpdFrm.Free ;
   end;
end;

procedure TSTMntPrtBinFrm.AddBitBtnClick(Sender: TObject);
begin
CallUpdScreen('A') ;
end;

procedure TSTMntPrtBinFrm.BinsListViewSelectItem(Sender: TObject;
  Item: TListItem; Selected: Boolean);
begin
CheckButtons(Self) ;
end;

procedure TSTMntPrtBinFrm.CheckButtons(Sender: TObject);
Var
ItempCount: Integer ;
...
ProductionBtn.Enabled := bTempSelected;
...
procedure TSTMntPrtBinFrm.DelBitBtnClick(Sender: TObject);
Var
iTempCount: Integer ;
begin
...
procedure TSTMntPrtBinFrm.UpdBitBtnClick(Sender: TObject);
Var
TempWord: Word ;
iTempCount: Integer ;
begin
...
procedure TSTMntPrtBinFrm.SelBitBtnClick(Sender: TObject);
begin
Close;
end;

procedure TSTMntPrtBinFrm.BinsListViewClick(Sender: TObject);
begin
CheckButtons(Self) ;
end;

procedure TSTMntPrtBinFrm.FormShow(Sender: TObject);
begin
If bAllow_Upd then
   begin
   Caption := 'Maintain Store Bins';
   selbitbtn.Visible := False;
   end
else
   begin
...
procedure TSTMntPrtBinFrm.ProductionbtnClick(Sender: TObject);
Var
TempWord: Word ;
iTempCount: Integer ;
begin
TempWord := MessageDlg('Do you want the selected bins to be production bins?', mtConfirmation,
               [mbCancel,mbNo,mbYes],0) ;
If (TempWord = mrNo) or (TempWord = mrYes) then
   begin
   For iTempCount := 0 to BinsListView.Items.Count - 1 do
