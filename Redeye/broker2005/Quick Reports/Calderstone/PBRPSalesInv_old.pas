...
var
  ...
  RegAddress : array [0..9] of TStringList;
...
*)
  ...
  {This is where we define the logo to print depending on the invoice location/revenue centre}
  if InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger <> iInvoiceLocation then
    begin
      if assigned(Images[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].picture.graphic) then
        begin
          if NOT Images[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].picture.graphic.empty then
            imgReport.Picture := Images[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].Picture;
        end;

      {Set Payment details}
      if Payments[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].text <> '' then
        begin
          memPayment.Lines.Clear;
          memPayment.lines.Text := Payments[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].text;
        end;

      {Set Registered Address details}
      if RegAddress[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].text <> '' then
        begin
          qrlblCompanyReg.lines.clear;
          qrlblCompanyReg.lines.text := RegAddress[InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger].text;
        end;

      iInvoiceLocation := InvHeadSRC.Dataset.FieldByName('Invoice_Location').Asinteger;
    end;
end;
  ...
  for iCount := 0 to 9 do
    begin
      try
        ...
        RegAddress[iCount] := TStringList.Create;
      except
      end;
    end;
    ...
    begin
      ...
      while EOF <> true do
        begin
          ...
          RegAddress[fieldbyname('Invoice_Location').asinteger] := TStringList.Create;
          if fieldbyname('Registered_Address').asstring <> '' then
            RegAddress[fieldbyname('Invoice_Location').asinteger].text := fieldbyname('Registered_Address').asstring
          else
            RegAddress[fieldbyname('Invoice_Location').asinteger].text := qrlblCompanyReg.lines.Text;
          next;
        end;
    end;
...
procedure TPBRPSalesInvFrm.FormDestroy(Sender: TObject);
var
  iCount: integer;
begin
  for iCount := 0 to 9 do
