...
VSS Info:
$Header: /PBL D5/Quick Reports/PMS/PBRPDeliv.pas 8     28/02/:3 14:45 Paul $
 ...
 * 
 * *****************  Version 7  *****************
 * User: Janiner      Date: 3/10/:2    Time: 16:07
 * Updated in $/PBL D5/Quick Reports/PMS
 * new format of deliv note
 * 
 * *****************  Version 6  *****************
 ...
 * 
 * *****************  Version 4  *****************
 * User: Andrewh      Date: 17/09/02   Time: 14:49
 * Updated in $/PBL D5/Quick Reports/PMS
...
*)
  ...
  if assigned(Images[CustDetsSQL.FieldByName('Invoice_Location').Asinteger].picture.graphic) then
    begin
      if NOT Images[CustDetsSQL.FieldByName('Invoice_Location').Asinteger].picture.graphic.empty then
        ReportImage.Picture := Images[CustDetsSQL.FieldByName('Invoice_Location').Asinteger].Picture;
    end;
end;
...
procedure TPBRPDelivFrm.FormDestroy(Sender: TObject);
var
  iCount: integer;
begin
  for iCount := 0 to 9 do
