...
*)
  ...
  if (PODelivSQL.FieldByName('Customer').AsString <> '') or (PODelivSQL.FieldByName('Supplier').AsString <> '') or (PODelivSQL.FieldByName('Ad_Hoc_Address').AsString <> '') then
    begin
...
begin
  FbAddressOnly := Value;
  DescrQRLabel.enabled := not FbAddressOnly;
  FormReflbl.enabled := not FbAddressOnly;
  QRLabel1.enabled := not FbAddressOnly;
  QRLabel3.enabled := not FbAddressOnly;
