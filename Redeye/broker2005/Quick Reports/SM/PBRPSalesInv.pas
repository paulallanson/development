...
uses
  ...
  CCSCommon, PBPOObjects, gtQrCtrls, gtQrExport, qrprntr, printers, jpeg;
...
type
  TPBRPSalesInvFrm = class(TForm)
    ...
    VatPerclbl: TgtQRLabel;
    qryUpCustomer: TQuery;
    qryUpInvoicePay: TQuery;
    ...
    qryGetCustomerPaymentTerms: TQuery;
...
uses pbMainMenu, PBImages, pbDatabase, PBAuditDM;
...
//    ParamByName('Sales_Invoice_No').AsString := IntToStr(FInvoiceNo);
    ...
    PBAuditDataMod.WriteAudit(4300, InvHeadSRC.dataset.FieldByName('Sales_Invoice').AsInteger, strtoint(InvoiceNumberLbl.Caption), 0, 0, InvoiceNumberLbl.Caption) ;
  end;
...
begin
  ...
  bCustomerIsReseller := (CustomerSQL.fieldbyname('Customer_is_Acquired').asstring = 'Y');
