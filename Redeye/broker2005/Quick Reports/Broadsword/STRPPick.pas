...
uses
  ...
  Db, DBTables, QuickRpt, Qrctrls, ExtCtrls, StdCtrls, STSOObjects, STPacks, CCSPrint, qrprntr, printers;
...
type
  TSTRPPickfrm = class(TForm)
    ...
    QRLabel15: TQRLabel;
    qrlblPartGroup: TQRLabel;
    GetPartGrpSQL: TQuery;
...
*)
  ...
  PickSOListQuickReport.PrinterSettings.Copies := Copies;   {set the output bin the }
end;
...
begin
  ...
  with GetPartGrpSQL do
    begin
      close;
      parambyname('Part_Group').asinteger := GetAllocDetQuery.FieldbyName('Part_Group').Asinteger;
      open;
      qrlblPartGroup.caption := fieldbyname('Part_group_Descr_Short').asstring;
    end;
