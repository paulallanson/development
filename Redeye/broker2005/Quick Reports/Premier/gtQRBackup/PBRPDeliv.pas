...
VSS Info:
$Header: /PBL D5/Quick Reports/Horner Bros/PBRPDeliv.pas 1     19/06/:3 14:48 Paul $
$History: PBRPDeliv.pas $
 * 
 * *****************  Version 1  *****************
 * User: Paul         Date: 19/06/:3   Time: 14:48
 * Created in $/PBL D5/Quick Reports/Horner Bros
 * Initial take on of Horner Bros units
 * 
 * *****************  Version 3  *****************
 * User: Paul         Date: 28/02/:3   Time: 14:44
 * Updated in $/PBL D5/Quick Reports/ImageForms
 * sets PrinterSettings.outputbin to user selected details 
 * 
 * *****************  Version 2  *****************
 * User: Paul         Date: 15/11/:2   Time: 11:23
 * Updated in $/PBL D5/Quick Reports/ImageForms
 * Reformatted reports to Imageforms layout
 * 
 * *****************  Version 4  *****************
...
type
  TPBRPDelivFrm = class(TForm)
    ...
    PONumberLbl: TPsQRLabel;
    AddressMemo: TPsQRMemo;
    DeliveryMemo: TPsQRMemo;
    BoxesLbl: TPsQRLabel;
    ...
    YourRefLbl: TPsQRLabel;
    DateLbl: TPsQRLabel;
    OrderRefLbl: TPsQRLabel;
    QtyOrderedLbl: TPsQRLabel;
    GetNarrSQL: TFDQuery;
    CompSQL: TFDQuery;
    Account: TPsQRDBText;
    CustDetsSQL: TFDQuery;
    Accountlbl: TPsQRLabel;
    DelInstructMemo: TPsQRMemo;
    FormRefLbl: TPsQRLabel;
    FormRefDescLbl: TPsQRLabel;
    DeliveryDateLbl: TPsQRLabel;
    QRLabel1: TPsQRLabel;
    lblDelInst: TPsQRLabel;
    QRLabel3: TPsQRLabel;
    QRLabel4: TPsQRLabel;
    QRLabel5: TPsQRLabel;
    QRLabel6: TPsQRLabel;
    ...
    QRLabel14: TPsQRLabel;
    QRLabel12: TPsQRLabel;
    PsQRFilters1: TPsQRFilters;
...
uses PBMenuMain, PBPODataMod, CCSCommon;
...
var
  ...
  AFilters: TPsQRFilters;
  RTFFilter: TPsQRRTFFilter;
  HTMLFilter: TPsQRHTMLFilter;
  PDFFilter: TPsQRPDFFilter;
  BMPFilter: TPsQRBMPFilter;
  GIFFilter: TPsQRGIFFilter;
  JPEGFilter: TPsQRJPEGFilter;
  i: integer;
begin
  ...
  AFilters := TPsQRFilters.Create(self);
  ...
  begin
    GIFFilter := TPsQRGIFFilter.Create(fileName);
    try
      PBDelivQuickReport.Prepare;
  ...
  begin
    BMPFilter := TPsQRBMPFilter.Create(fileName);
    try
      PBDelivQuickReport.Prepare;
  ...
  begin
    HTMLFilter := TPsQRHTMLFilter.Create(fileName);
    try
      PBDelivQuickReport.Prepare;
  ...
  begin
    JPEGFilter := TPsQRJPEGFilter.Create(fileName);
    try
      PBDelivQuickReport.Prepare;
