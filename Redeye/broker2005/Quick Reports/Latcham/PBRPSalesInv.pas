...
uses
  ...
  CCSCommon, PBPOObjects, gtQrCtrls, gtQrExport, qrprntr, printers;
...
type
  TPBRPSalesInvFrm = class(TForm)
    ...
    lblInvoiceDescription: TgtQRLabel;
    ...
    qrlblFSC: TgtQRLabel;
    qrlblcostCentreLabel: TgtQRLabel;
    qrlblCostCentre: TgtQRLabel;
  ...
  private
    ...
    function GetJBCustCostCentre(tempCode: string): string;
    function GetPOCustCostCentre(tempCode: string): string;
    function GetSOCustCostCentre(tempCode: string): string;
...
*)
//  imgReport.Picture := PBImagesFrm.ReportImage.Picture;
//  imgReport.Picture.bitmap.pixelformat := pfDevice;
  imgReport.Enabled := true;
...
begin
  ...
  memPayment.Height := (memPayment.Lines.Count * 13);
  qrshpPayment.Height := (memPayment.Lines.Count * 13) + 14;
...
*)
  if trim(gtlblFSCClaim.caption) = '' then
    ...
    begin
      ...
      chldbndFSCClaim.enabled := not (InvLineSRC.Dataset.fieldbyname('Not_Printed').asstring = 'Y');
    end;
...
begin
  ...
  if InvHeadSRC.Dataset.FieldByName('Category').asinteger <> 0 then
    begin
      memPayment.Alignment := taLeftJustify;
      BuildPaymentNotes(getCategoryPayNotes(InvHeadSRC.Dataset.FieldByName('Category').asinteger));
    end
  else
    begin
      memPayment.lines.clear;
      memPayment.Alignment := taCenter;
      BuildPaymentNotes(dmBroker.GetCompanyPaymentNotes);
(*      for ilines := 0 to pred(memDefPayment.lines.count) do
        begin
          memPayment.lines.Add(memDefPayment.Lines.Strings[iLines]);
        end;
*)    end;
  ...
  {Get the Cost Centre}
  if InvHeadSRC.Dataset.FieldByName('Sales_Invoice_type').Asstring = 'S' then
    qrlblCostCentre.caption := GetSOCustCostCentre(InvHeadSRC.Dataset.FieldByName('Reference').Asstring)
  else
  if InvHeadSRC.Dataset.FieldByName('Sales_Invoice_type').Asstring = 'J' then
    qrlblCostCentre.caption := GetJBCustCostCentre(InvHeadSRC.Dataset.FieldByName('Reference').Asstring)
  else
    qrlblCostCentre.caption := GetPOCustCostCentre(InvHeadSRC.Dataset.FieldByName('Reference').Asstring);

  qrlblCostCentre.enabled := (trim(qrlblCostCentre.caption) <> '');
  qrlblCostCentreLabel.Enabled := qrlblCostCentre.enabled;
...
function TPBRPSalesInvFrm.GetJBCustCostCentre(tempCode: string): string;
begin
  result := '';
  if not bInvoice then exit;
  with qryJBHead do
...
function TPBRPSalesInvFrm.GetPOCustCostCentre(tempCode: string): string;
begin
  result := '';
  if not bInvoice then exit;
  with qryPOLine do
...
function TPBRPSalesInvFrm.GetSOCustCostCentre(tempCode: string): string;
begin
  result := '';
  if not bInvoice then exit;
  with qrySOHead do
