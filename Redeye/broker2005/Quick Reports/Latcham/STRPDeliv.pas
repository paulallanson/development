...
type
  ...
  private
    ...
    procedure BuildHeaderDeliveryNotes(aQuery: TQuery;
...
begin
  ...
  qrmCompany1.lines.clear;
  qrmCompany1.enabled := PrintLogo;
  ...
  begin
    ...
    for irow := 1 to 5 do
      begin
        if Trim(qryCompName.Fields[irow].AsString) <> '' then
          sAddress := sAddress + Trim(qryCompName.Fields[irow].AsString) + ' ';
      end;
    Lines.Append(Trim(sAddress)) ;
    {Finally, add the phone number} ;
    Lines.Append('T ' + Trim(qryCompName.FieldByName('Phone').AsString) + ' F ' + Trim(qryCompName.FieldByName('Fax_Number').AsString));
    Lines.Append('www.latchamdirect.co.uk');
  end;

  for irow := 0 to pred(qrmCompany.lines.count) do
    qrmCompany1.Lines.Append(qrmCompany.lines[irow]);
...
//      QRChldBndCour.AlignToBottom := True;
  ...
  begin
    ...
    qrmCompany1.Enabled := true;
    qrlblVatNo1.Enabled := true;
  end
  else
  begin
    ReportImage.Enabled := false;
    qrmCompany.Enabled := false;
    qrlblVatNo.Enabled := false;
    qrmCompany1.Enabled := false;
    qrlblVatNo1.Enabled := false;
  end
end;
...
procedure TSTRPDelivfrm.BuildHeaderDeliveryNotes(aQuery: TQuery;
  const iNarrative: integer);
var
  aStr : string;
begin
