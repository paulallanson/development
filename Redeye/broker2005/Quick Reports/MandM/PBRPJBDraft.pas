...
type
  TPBRPJBDraftFrm = class(TForm)
    ...
    lblSellPrice: TgtQRLabel;
    lblSellUnit: TgtQRLabel;
    lblValue: TgtQRLabel;
    ...
    SalesOrdDateQRLabel: TgtQRLabel;
    CustomerAddmemo: TgtQRMemo;
    QRLblWho: TgtQRLabel;
    QRLblCntct: TgtQRLabel;
    QRLblCustRef: TgtQRLabel;
    QRLblRef: TgtQRLabel;
    QRLblReqDt: TgtQRLabel;
    QRLblDtReq: TgtQRLabel;
    ...
    qrySOLine: TQuery;
    QRLabel1: TgtQRLabel;
    lblJobBagDescription: TgtQRLabel;
    qrsbdAddCharges: TQRSubDetail;
    lblAddDescription: TgtQRLabel;
    lblAddValue: TgtQRLabel;
    lblAddVAT: TgtQRLabel;
    qryJBLineChgs: TQuery;
    lblQuantity: TgtQRLabel;
    qrmDescription: TgtQRRichText;
    qryPOLineChgs: TQuery;
    gtQRFilters1: TgtQRFilters;
    qrlblRevenueCentre: TgtQRLabel;
    qrlblRevenueCentreData: TgtQRLabel;
...
uses PBSendFax, pbMainMenu, CCSCommon, PBImages, pbDatabase;

var
  iGoods, iVAT: Real;
  Images : array [0..9] of TImage;
  Payments : array [0..9] of TStringList;
...
//    lblDescription.Caption := FieldByName('Job_Bag_line_Descr').AsString;
    if FinanceVers then
      ...
      qrmDescription.lines.add(trim(Fieldbyname('Job_Bag_line_descr').asstring));
...
//    lblSellPrice.Caption := formatfloat('0.000',SellPrice);
    ...
    lblSellPrice.Caption := CurrencyDisp('£###,##0.00',FloatToStr(SellPrice));

    lblSellUnit.Caption := PriceUnitDescr;
    if PriceUnitFactor = 0 then
      lblValue.Caption := CurrencyDisp('£###,##0.00',FloatToStr(SellPrice))
    else
      lblValue.Caption := CurrencyDisp('£###,##0.00',FloatToStr((Quantity * SellPrice)/PriceUnitFactor));

    sValue := stringreplace(lblValue.Caption,'£','',[rfReplaceAll]);
    sValue := stringreplace(sValue,',','',[rfReplaceAll]);
    iGoods := iGoods + StrToFloat(sValue);

    rVatValue := StrToFloat(sValue) * (FieldByName('Vat_Rate').AsFloat / 100);
...
//  sEmailQRLabel.Caption := addressSrc.Dataset.fields[8].AsString;
  ...
  try
    ...
    {Set Payment details}
    if Payments[GetDetsQuery.FieldByName('Invoice_Location').Asinteger].text <> '' then
      begin
        memPayment.Lines.Clear;
        memPayment.lines.Text := Payments[GetDetsQuery.FieldByName('Invoice_Location').Asinteger].text;
      end;
  except
  end;
end;
...
procedure TPBRPJBDraftFrm.BuildPaymentNotes(const iNarrative : integer);
var
  aStr : string;
begin
...
*)
  ...
  lblAddValue.caption := CurrencyDisp('£###,##0.00',FloatToStr(qryPOLineChgs.fieldbyname('Quotation_Price').asfloat));

  sValue := stringreplace(lblAddValue.caption,'£','',[rfReplaceAll]);
  sValue := stringreplace(sValue,',','',[rfReplaceAll]);

  if qryPOLineChgs.FieldByName('Vat_Rate').Asstring = '' then
    rVatValue := StrToFloat(sValue) * (qryPOLineChgs.FieldByName('Customer_Vat_Rate').AsFloat / 100)
  else
    rVatValue := StrToFloat(sValue) * (qryPOLineChgs.FieldByName('Vat_Rate').AsFloat / 100);
...
procedure TPBRPJBDraftFrm.FormCreate(Sender: TObject);
var
  iCount: integer;
begin
  for iCount := 0 to 9 do
    begin
      try
        ...
        Payments[iCount] := TStringList.Create;
      except
      end;
    end;
    ...
    begin
      ...
      while EOF <> true do
        begin
          ...
          Payments[fieldbyname('Invoice_Location').asinteger] := TStringList.Create;
          if fieldbyname('Invoice_Payment_Notes').asinteger <> 0 then
            Payments[fieldbyname('Invoice_Location').asinteger].text := dmBroker.GetNarrative(fieldbyname('Invoice_Payment_Notes').asinteger);
...
procedure TPBRPJBDraftFrm.FormDestroy(Sender: TObject);
var
  iCount: integer;
begin
  for iCount := 0 to 9 do
