...
type
  TPBRPPEndRepsPerformfrm = class(TForm)
    ...
    qrlblComm8: TQRLabel;
    qrlblComm9: TQRLabel;
    qrlblComm10: TQRLabel;
    qrlblComm11: TQRLabel;
    qrlblComm12: TQRLabel;
    qrlblCommTotal: TQRLabel;
    QRShape1: TQRShape;
    ...
    qrlblTotalComm1: TQRLabel;
    qrlblTotalComm2: TQRLabel;
    qrlblTotalComm3: TQRLabel;
    qrlblTotalComm4: TQRLabel;
    qrlblTotalComm5: TQRLabel;
    qrlblTotalComm6: TQRLabel;
    qrlblTotalComm7: TQRLabel;
    qrlblTotalComm8: TQRLabel;
    qrlblTotalComm9: TQRLabel;
    qrlblTotalComm10: TQRLabel;
    qrlblTotalComm11: TQRLabel;
    qrlblTotalComm12: TQRLabel;
    ...
    QRLabel50: TQRLabel;
    QRLabel51: TQRLabel;
    qrdbRepNameTotal: TQRDBText;
    procedure quickreportBeforePrint(Sender: TCustomQuickRep;
      var PrintReport: Boolean);
    procedure qrRepBandBeforePrint(Sender: TQRCustomBand;
    ...
    procedure qrbGrpRepFooterBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
    procedure qrbCompanyTotalBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
...
var
  PBRPPEndRepsPerformfrm: TPBRPPEndRepsPerformfrm;
...
function TPBRPPEndRepsPerformfrm.GetDetails(Sender: TObject): integer;
begin
  ...
  with qryReps do
    begin
      close;
      case RepType of
        1: SQL.Text := qrySubReps.SQL.Text;
      else
        SQL.Text := qryReps.SQL.text;
      end;
...
procedure TPBRPPEndRepsPerformfrm.quickreportBeforePrint(
...
begin
  ...
  if ReportType = 1 then
    begin
//      qrRepBand.ForceNewPage := false;
//      qrbCompanyTotal.ForceNewPage := false;
      qrRepBand.frame.DrawTop := true;
      qrRepBand.frame.Drawbottom := false;
      qrbGrpRepFooter.Frame.DrawTop := false;
      qrdbRepNameTotal.enabled := false;
    end;
...
procedure TPBRPPEndRepsPerformfrm.qrRepBandBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Rep := qryReps.fieldbyname('Rep').asinteger;
end;

procedure TPBRPPEndRepsPerformfrm.QRSubDetail1BeforePrint(
    ...
    begin
      with qryReport do
        begin
          close;
          case RepType of
            1: SQL.Text := qrySubReportNull.SQL.Text;
          else
            SQL.Text := qryReportNull.SQL.text;
          end;
    ...
    begin
      with qryReport do
        begin
          close;
          case RepType of
            1: SQL.Text := qrySubReport.SQL.Text;
          else
...
procedure TPBRPPEndRepsPerformfrm.qrsubPeriodsBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  tempPeriod: integer;
begin
...
procedure TPBRPPEndRepsPerformfrm.qrgRepsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
begin
...
procedure TPBRPPEndRepsPerformfrm.qrbGrpCatFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
...
// Create Actual profit Totals for the Rep
  ...
  PrintBand := self.ReportType = 0;
end;

procedure TPBRPPEndRepsPerformfrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndRepsPerformfrm.qrbGrpRepFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
  ...
  rCommission: real;
begin
// Get the commission rates for this rep
  with qryRep do
    begin
      close;
      ...
      rCommission := fieldbyname('Commission_Rate').asfloat;
      qrlblCommission.Caption := formatfloat('##0.00%',rCommission) + ' Commission';
      qrlblCommission.enabled := ShowCommission
    end;

// Get the budgets for this rep
  with qryRepBudgets do
    begin
      close;
      parambyname('Rep').asinteger := Rep;
      parambyname('Start_Period').asinteger := StartPeriod;
...
// Show Rep Total Commission
  rRepActualCommTotal := 0;
  for i := 1 to 24 do
    begin
      rRepActualCommTotal := rRepActualCommTotal + (RepProfit[i]*(rCommission/100));
      qrlblCommTotal.caption := formatfloat('#,##0',rRepActualCommTotal);
      qrlblCommTotal.enabled := ShowCommission;
    end;
...
procedure TPBRPPEndRepsPerformfrm.qrbCompanyTotalBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
// Show total profit
  ...
  for i := 1 to 24 do
    begin
      rActualCommTotal := rActualCommTotal + TotalComm[i];
    end;
  qrlblTotalComm.caption := formatfloat('#,##0',rActualCommTotal);
  qrlblTotalComm.enabled := ShowCommission;
...
// Show TOTAL % Difference in Actual Sales and Budget sales
  ...
  qrlblTotalCommission.enabled := ShowCommission;

end;

procedure TPBRPPEndRepsPerformfrm.SetExcludeJBCosts(const Value: boolean);
begin
  FExcludeJBCosts := Value;
  qrlblExcludeCosts.Enabled := FExcludeJBCosts;
end;

procedure TPBRPPEndRepsPerformfrm.ExportToFile(fileName: string);
var
  tempStr: string;
  i: integer;
  tmpLabel: string;
...
procedure TPBRPPEndRepsPerformfrm.qrbGrpCatFooterAfterPrint(
...
procedure TPBRPPEndRepsPerformfrm.SetBudgetType(const Value: integer);
begin
  FBudgetType := Value;
end;

procedure TPBRPPEndRepsPerformfrm.SetShowCommission(const Value: boolean);
begin
  FShowCommission := Value;
end;

procedure TPBRPPEndRepsPerformfrm.SetRepType(const Value: integer);
begin
  FRepType := Value;
end;

procedure TPBRPPEndRepsPerformfrm.SetReportType(const Value: integer);
begin
  FReportType := Value;
end;
