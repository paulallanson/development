...
type
  ...
  Private
    ...
    procedure SetPurch_Price(const Value: real);
  Public
    ...
    ByPart: Bytebool; Purch_Price: real);
  ...
  property Purch_Price: real read ddPurch_Price write SetPurch_Price;
  ...
  var
  PBLUCustDiscFrm: TPBLUCustDiscFrm;

implementation

uses PBMaintCustDisc, pbDatabase, pbMainMenu, PBLUSupp;

{$R *.DFM}

procedure TPBLUCustDiscFrm.AddBitBtnClick(Sender: TObject);
begin
{Add a new Rep} ;
CallMaintScreen('A') ;
end;

procedure TPBLUCustDiscFrm.ChgBitBtnClick(Sender: TObject);
begin
{Change a Rep} ;
  if rec_point = 0 then
    exit;
...
procedure TPBLUCustDiscFrm.DelBitBtnClick(Sender: TObject);
begin
{Delete a Rep} ;
if rec_point = 0 then
  exit;
...
procedure TPBLUCustDiscFrm.CallMaintScreen(sTempFuncMode: String);
begin
  PBMaintCustDiscFrm := TPBMaintCustDiscFrm.Create(self) ;
  try
    PBMaintCustDiscFrm.sFuncMode := sTempFuncMode ;
    PBMaintCustDiscFrm.ShowModal ;
    if not (PBMaintCustDiscFrm.ModalResult = mrCancel) then
      UpdateTreeView(sTempFuncMode) ;
  finally
    PBMaintCustDiscFrm.Free ;
  end;
end;

procedure TPBLUCustDiscFrm.LoadTreeViews(Sender: TObject);
Var
scode: string;
begin
...
ddTemp := TDiscDet.Create(0, '', '', '', 0, 0, 0.000, 0, 0, True, 0.000);
tncode.Data := ddTemp;
tngroup := DiscTreeView.Items.Add(Nil,'Product Group');
ddTemp := TDiscDet.Create(0, '', '', '', 0, 0, 0.00, 0, 0, False, 0.000);
tngroup.data := ddTemp;
...
*)
          ...
          ddTemp := TDiscDet.Create(fieldByName('Customer_Discount').AsInteger, fieldByName('Discount_Type').AsString,
            ...
            fieldByName('Date_Ineffective').AsDateTime, True, fieldByName('Part_Purchase_Price').AsFloat);
          tnTemp.Data := ddtemp;
          Next;
          end;
      end;
...
*)
      ...
      ddTemp := TDiscDet.Create(fieldByName('Customer_Discount').AsInteger, fieldByName('Discount_Type').AsString,
          ...
          fieldByName('Date_Ineffective').AsDateTime, False, fieldByName('Part_Purchase_Price').AsFloat);
      tntemp.data := ddTemp;
      Next;
      end;
...
procedure TPBLUCustDiscFrm.FormActivate(Sender: TObject);
begin
...
procedure TPBLUCustDiscFrm.UpdateTreeView(sTempFuncMode: String);
begin
...
procedure TPBLUCustDiscFrm.DiscTreeViewClick(Sender: TObject);
begin
...
constructor TDiscDet.Create(const Rec: Integer; Typ, Desc, Part: String;
  ...
  Date_ineffec: TDatetime; ByPart: Bytebool; Purch_Price: real);
begin
DDRec := Rec;
DDTyp := Typ;
DDDesc := Desc;
...
procedure TDiscDet.SetPurch_Price(const Value: real);
begin
DDPurch_Price := Value;
end;
...
procedure TPBLUCustDiscFrm.FormDestroy(Sender: TObject);
begin
...
procedure TPBLUCustDiscFrm.FormCreate(Sender: TObject);
begin
  edtSupplier.text := '';
  iSupp := 0;
...
procedure TPBLUCustDiscFrm.btnSupplierClick(Sender: TObject);
begin
...
procedure TPBLUCustDiscFrm.btnClearClick(Sender: TObject);
begin
  edtSupplier.text := '';
  iSupp := 0;
  LoadTreeViews(Self);
