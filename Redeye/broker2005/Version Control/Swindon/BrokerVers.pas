...
begin
  ...
  begin
    MessageDlg('The Broker application is already running', mtError,
      [mbAbort], 0);
	...
	ServDir := '\\DEMETER\POLLUX\BROKER$' ;

  LocalDrive := copy(GetWinSysDir,1,2);
	LocalDir := LocalDrive + '\Broker Applications';
  ...
  {Test that the Backup Directory exists, if not then contruct}
  if not DirectoryExists(BackupDir) then
  	begin
     	CreateBrokerDir(BackupDir);
  	end;
	BrokerVersFrm.Refresh ;

	{Backup the old program program} ;
	StatusNarr('Backing up Broker') ;
	BackupIfNewer('Broker2005.exe', 'Broker Program') ;
	...
	StatusNarr('Checking Costing Program') ;
	CopyIfNewer('costingtool.xls', 'Costing Program') ;
...
procedure TBrokerVersFrm.CopyIfNewerINI(FName, FNewName, FDesc: String);
begin
  {Check if server version of file is older or same. If it is don't copy it} ;
