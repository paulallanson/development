...
//	ServDir := '\\NEXUS-UK\Redeye' ;
  ...
  {Test that the Vision Directory exists, if not then contruct}
  if not DirectoryExists(LocalDir+'\Redeye_Vision') then
  	begin
     	CreateBrokerDir(LocalDir+'\Redeye_Vision\');
  	end;

  {Test that the Vision Directory exists, if not then contruct}
  if not DirectoryExists(LocalDir+'\Redeye_NexFoto') then
  	begin
     	CreateBrokerDir(LocalDir+'\Redeye_NexFoto\');
  	end;

  {Test that the Vision Directory exists, if not then contruct}
  if not DirectoryExists(LocalDir+'\Redeye_Kiwe3') then
  	begin
     	CreateBrokerDir(LocalDir+'\Redeye_Kiwe3\');
  	end;

  {Test that the NexPack Directory exists, if not then contruct}
  if not DirectoryExists(LocalDir+'\Redeye_NexPack') then
  	begin
     	CreateBrokerDir(LocalDir+'\Redeye_NexPack\');
  	end;
	...
	{Test the Vision bitmaps} ;
	StatusNarr('Checking Logos') ;
	CopyIfNewer('Redeye_Vision\PBRepLogo.Bmp', 'Report Logo') ;

	{Test the NexFoto bitmaps} ;
	StatusNarr('Checking Logos') ;
	CopyIfNewer('Redeye_NexFoto\PBRepLogo.Bmp', 'Report Logo') ;

	{Test the Kiwe3 bitmaps} ;
	StatusNarr('Checking Logos') ;
	CopyIfNewer('Redeye_Kiwe3\PBRepLogo.Bmp', 'Report Logo') ;

	{Test the NexPack bitmaps} ;
	StatusNarr('Checking Logos') ;
	CopyIfNewer('Redeye_NexPack\PBRepLogo.Bmp', 'Report Logo') ;
