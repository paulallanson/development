...
type
  TPBRPPEndRepsCatPerformfrm = class(TForm)
    ...
    qrlblNewBusiness: TQRLabel;
    qrlblNewTO1: TQRLabel;
    qrlblNewProf1: TQRLabel;
    qrlblNewTO2: TQRLabel;
    qrlblNewProf2: TQRLabel;
    qrlblNewTO3: TQRLabel;
    qrlblNewProf3: TQRLabel;
    qrlblNewTO4: TQRLabel;
    qrlblNewProf4: TQRLabel;
    qrlblNewTO5: TQRLabel;
    qrlblNewProf5: TQRLabel;
    qrlblNewTO6: TQRLabel;
    qrlblNewProf6: TQRLabel;
    qrlblNewTO7: TQRLabel;
    qrlblNewProf7: TQRLabel;
    qrlblNewTO8: TQRLabel;
    qrlblNewProf8: TQRLabel;
    qrlblNewTO9: TQRLabel;
    qrlblNewProf9: TQRLabel;
    qrlblNewTO10: TQRLabel;
    qrlblNewProf10: TQRLabel;
    qrlblNewTO11: TQRLabel;
    qrlblNewProf11: TQRLabel;
    qrlblNewTO12: TQRLabel;
    qrlblNewProf12: TQRLabel;
    qrlblCatNewTOTotal: TQRLabel;
    qrlblCatNewProfTotal: TQRLabel;
    qryRepCat: TQuery;
...
var
  PBRPPEndRepsCatPerformfrm: TPBRPPEndRepsCatPerformfrm;

implementation

uses PBRSPEndRepsPerform;

{$R *.DFM}

function TPBRPPEndRepsCatPerformfrm.GetDetails(Sender: TObject): integer;
begin
  with qryReps do
    begin
      close;
...
procedure TPBRPPEndRepsCatPerformfrm.quickreportBeforePrint(
...
procedure TPBRPPEndRepsCatPerformfrm.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Rep := qryReps.fieldbyname('Rep').asinteger;
end;

procedure TPBRPPEndRepsCatPerformfrm.QRSubDetail1BeforePrint(
    ...
    begin
      ...
      while eof <> true do
        begin
          ...
          case BudgetType of
            0:  categoryProfitBudget[i] := fieldbyname('Operating_Profit_Value').asfloat;
            1:  categoryProfitBudget[i] := fieldbyname('Profit_Value').asfloat;
          end;
...
procedure TPBRPPEndRepsCatPerformfrm.qrsubPeriodsBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  tempPeriod: integer;
begin
...
procedure TPBRPPEndRepsCatPerformfrm.qrgRepsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
begin
...
procedure TPBRPPEndRepsCatPerformfrm.qrbGrpCatFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
  rNewBusiness: real;
  ...
  templbl: TQRLabel;
begin
  {Get the Rep Category New Business Rate}
  with qryRepCat do
    begin
      close;
      parambyname('Financial_Year').asinteger := FinancialYear;
      parambyname('Rep').asinteger := qryReps.fieldbyname('Rep').asinteger;
      parambyname('Category').asinteger := qryCategory.fieldbyname('Category').asinteger;
      open;

      rNewBusiness := fieldbyname('New_Business_Rate').asfloat;
      qrlblNewBusiness.Caption := 'New Business - ' + formatfloat('##0%',rNewbusiness);
    end;
  ...
  for i := 1 to 24 do
    begin
      templbl := ((FindComponent('qrlblNewTO' + IntToStr(i))) as TQRLabel);
      try
        templbl.Caption := formatfloat('#,##0',(categorySalesBudget[i]*(rNewBusiness/100)));
      except
      end;
    end;
  ...
  for i := 1 to 24 do
    begin
      templbl := ((FindComponent('qrlblNewProf' + IntToStr(i))) as TQRLabel);
      try
        templbl.Caption := formatfloat('#,##0',(categoryProfitBudget[i]*(rNewBusiness/100)));
      except
      end;
    end;
...
// Show Category Budget Sales
  ...
  for i := 1 to 24 do
    begin
      rNewSalesTotal := rNewSalesTotal + (categorySalesBudget[i] * (rNewBusiness/100));
      qrlblCatNewTOTotal.caption := formatfloat('#,##0',rNewSalesTotal);
    end;
...
// Show Category total Sales
  ...
  for i := 1 to 24 do
    begin
      rNewProfitTotal := rNewProfitTotal + (categoryProfitBudget[i] * (rNewBusiness/100));
      qrlblCatNewProfTotal.caption := formatfloat('#,##0',rNewProfitTotal);
    end;
...
// Create Budget sales Totals for the Rep
  for i := 1 to 24 do
    begin
      RepSalesBudget[i] := RepSalesBudget[i] + categorySalesBudget[i] + (categorySalesBudget[i] * (rNewBusiness/100));
    end;

// Create Budget profit Totals for the Rep
  for i := 1 to 24 do
    begin
      RepProfitBudget[i] := RepProfitBudget[i] + categoryProfitBudget[i] + (categoryProfitBudget[i] * (rNewBusiness/100));
    end;
end;

procedure TPBRPPEndRepsCatPerformfrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndRepsCatPerformfrm.qrbGrpRepFooterBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndRepsCatPerformfrm.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  i: integer;
  templbl: TQRLabel;
...
procedure TPBRPPEndRepsCatPerformfrm.SetExcludeJBCosts(
  const Value: boolean);
begin
  FExcludeJBCosts := Value;
end;

procedure TPBRPPEndRepsCatPerformfrm.ExportToFile(fileName: string);
var
  tempStr: string;
  i: integer;
  tmpLabel: string;
...
procedure TPBRPPEndRepsCatPerformfrm.qrbGrpCatFooterAfterPrint(
...
begin
  ...
  begin
    //Rep
    tempStr := '"' + qryReps.fieldbyname('Name').asstring + '"';
    ...
    writeln(self.exportFile, tempStr);
    PBRSPEndRepsPerformfrm.prgbrExport.StepIt;
  end;
end;

procedure TPBRPPEndRepsCatPerformfrm.SetBudgetType(const Value: integer);
begin
  FBudgetType := Value;
end;
