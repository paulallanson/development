object STRSSlwMvngStkRepfrm: TSTRSSlwMvngStkRepfrm
  Left = 240
  Top = 117
  ...
  object GetPartsSQL: TFDQuery
    ...
    SQL.Strings = (
      ...
      '    Rep.Name as RepName'
        ...
        'FROM (((((Part INNER JOIN Store_Stock ON Part.Part = Store_Stock' +
        '.Part)'
      
        '      INNER JOIN Part_Store ON Store_Stock.Part_Store = Part_Sto' +
        're.Part_Store)'
      
        '      Left JOIN Sales_Order ON Part_Store.Part_Store = Sales_Ord' +
        'er.Part_Store)'
      
        '      Left JOIN Sales_Order_line ON (Sales_Order.Sales_Order = S' +
        'ales_Order_line.Sales_Order) AND'
      '       (Part.Part = Sales_Order_line.Part))'
      
        #9'left join ((Customer_branch LEFT JOIN Reps_Branches ON (Custome' +
        'r_Branch.Branch_no = Reps_Branches.Branch_no) AND'
      '         (Customer_Branch.Customer = Reps_Branches.Customer))'
      
        #9#9'LEFT JOIN Customer ON Customer_Branch.Customer = Customer.Cust' +
        'omer)'
      
        '        on part.customer = Customer_Branch.Customer and Part.Bra' +
        'nch_No = Customer_Branch.Branch_No)'
      '         LEFT JOIN Rep ON Reps_Branches.Rep = Rep.Rep'
      
        'where     ((Sales_Order.Date_Required <= :Date) or (Sales_Order.' +
        'Date_Required is null)) and'
