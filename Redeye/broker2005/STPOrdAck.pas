...
type
  TSTPOrdAckFrm = class(TForm)
    ...
    SPhoneQRLabel: TgtQRLabel;
    SFaxQRLabel: TgtQRLabel;
    SEmailLegQRLabel: TgtQRLabel;
    SEmailQRLabel: TgtQRLabel;
...
function TSTPOrdAckFrm.GetDetails(Sender: TObject): Integer;
begin
...
function TSTPOrdAckFrm.RunReport(Sender: TObject): Integer;
begin
  Result := 0;
  StartReport(self);
...
procedure TSTPOrdAckFrm.StartReport(Sender: TObject);
begin
...
procedure TSTPOrdAckFrm.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
if (not Preview ) then
...
procedure TSTPOrdAckFrm.pogroupheaderBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  icount: integer;
begin
...
procedure TSTPOrdAckFrm.FormCreate(Sender: TObject);
begin
...
procedure TSTPOrdAckFrm.QRSubDetail1AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  TotOrdval := TotOrdval + (QtyOrd * GetDetsQuery.fieldByname('Purchase_Price').AsFloat/PackSize);
end;

procedure TSTPOrdAckFrm.FooterQRBandBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  OrderValQRLabel.Caption:= FormatFloat('£######0.00',Totordval)
end;

procedure TSTPOrdAckFrm.FooterQRBandAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
 Totordval := 0.
end;

procedure TSTPOrdAckFrm.PurchOrdQuickReportBeforePrint(
...
procedure TSTPOrdAckFrm.QRSubDetail2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  TotOrdval := TotOrdval + (qryExtraCharges.fieldByname('Amount').AsFloat);
end;

procedure TSTPOrdAckFrm.QRSubDetail2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  with qryExtraCharges do
  begin
...
procedure TSTPOrdAckFrm.BuildSpecialNotes(iNarrative : integer);
var
  aStr : string;
begin
