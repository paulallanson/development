...
type
  ...
  public
    ...
    sProduct: string[25];
    sDescription, sFormRef: string[50];
  end;

var
  frmSTMaintFormRef: TfrmSTMaintFormRef;
...
procedure TfrmSTMaintFormRef.FormActivate(Sender: TObject);
begin
  ...
  begin
    {Empty details}
    RefDescrEdit.text := sDescription;
    RefIDEdit.Text := '';
  end;
  StockRefEdit.text := sProduct;
...
procedure TfrmSTMaintFormRef.CheckOK(Sender: TObject);
begin
  {Enable/disable OK button}
  OKBitBtn.Enabled := (Trim(RefIDEdit.Text) <> '');
end;

procedure TfrmSTMaintFormRef.CancelBitBtnClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmSTMaintFormRef.RefDescrEditChange(Sender: TObject);
begin
  CheckOK(Self);
end;

procedure TfrmSTMaintFormRef.OKBitBtnClick(Sender: TObject);
begin
  if sFuncMode[1] in ['A','C'] then
  begin
    //if the item is stocked or the user has entered a stock reference
    //then check for duplication of stock reference
    if (StockRefEdit.text <> '') then
    begin
    ...
    begin
      ...
      ParamByName('Reorder_Level').Asinteger := 0;
...
procedure TfrmSTMaintFormRef.SaveMemoField(Sender: TObject);
begin
  sOldValue := (Sender as TMemo).Text;
end;

function TfrmSTMaintFormRef.FormatMemoValue(TempQty, TempFormat, TempError: string; TempNeg: ByteBool): string;
begin
  {Format the qunatity};
...
procedure TfrmSTMaintFormRef.RefIDEditChange(Sender: TObject);
begin
  CheckOK(Self);
end;

procedure TfrmSTMaintFormRef.AddZero;
begin
...
procedure TfrmSTMaintFormRef.DeleteZero;
begin
  with qryZero do
  begin
    SQL.Clear;
...
function TfrmSTMaintFormRef.GetNextKey: integer;
var
  aGuid: TGuid;
begin
...
procedure TfrmSTMaintFormRef.ChkBoxStockedItemClick(Sender: TObject);
begin
...
procedure TfrmSTMaintFormRef.FormCreate(Sender: TObject);
begin
...
procedure TfrmSTMaintFormRef.RefIDEditExit(Sender: TObject);
begin
...
function TfrmSTMaintFormRef.duplicatedStockRef(stockRef: string): boolean;
begin
...
procedure TfrmSTMaintFormRef.RefIDEditKeyPress(Sender: TObject;
  var Key: Char);
begin
  case ord(key) of
    34, 39, 44: Key := #0;
...
procedure TfrmSTMaintFormRef.StockRefEditKeyPress(Sender: TObject;
  var Key: Char);
begin
  case ord(key) of
    34, 39, 44: Key := #0;
...
procedure TfrmSTMaintFormRef.RefDescrEditKeyPress(Sender: TObject;
  var Key: Char);
begin
  case ord(key) of
    34, 39, 44: Key := #0;
...
procedure TfrmSTMaintFormRef.StockRefEditChange(Sender: TObject);
begin
  CheckOK(Self);
end;
