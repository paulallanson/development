...
uses
  ...
  PBPOObjects, taoFrmts, taoCntrr, Clipbrd, ComObj, AxCtrls, taoMapi, ActiveX, Menus,
...
procedure TPBMaintContractFrm.taoWinControl1SetDataPaste(Sender: TObject;
  Data: IInterface);
begin
  MyWinControlSetData(Data);
end;

procedure TPBMaintContractFrm.taoWinControl1SetDataTarget(Sender: TObject;
  Data: IInterface; X, Y: Integer);
begin
...
procedure TPBMaintContractFrm.taoWinControl1UpdateAction(Sender: TObject;
  Action: TtaoUpdateAction; var Enable: Boolean);
begin
...
procedure TPBMaintContractFrm.MyWinControlSetData(const Data: IUnknown);
const
...
begin
  if Supports(Data, ItaoCells, MyData) then
    begin
      MyPath := dmBroker.GetCompanyContractDirectory + '\' + floattostr(Contract.ContractNumber) +'\';
