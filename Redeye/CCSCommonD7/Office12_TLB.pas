...
// PASTLWTR : 1.2
// File generated on 16/05/2012 12:48:35 from Type Library described below.
...
//   Hint: Symbol 'Type' renamed to 'type_'
//   Hint: Parameter 'Type' of TabStop2.Type changed to 'Type_'
//   Error creating palette bitmap of (TCustomXMLSchemaCollection) : Access violation at address 009D5407 in module 'tlib70.bpl'. Read of address 00000000
// ************************************************************************ //
// *************************************************************************//
// NOTE:                                                                      
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  TCustomXMLSchemaCollectionProperties= class;
{$ENDIF}
  TCustomXMLSchemaCollection = class(TOleServer)
  private
    FIntf:        _CustomXMLSchemaCollection;
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    FProps:       TCustomXMLSchemaCollectionProperties;
    function      GetServerProperties: TCustomXMLSchemaCollectionProperties;
{$ENDIF}
  ...
  published
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
    property Server: TCustomXMLSchemaCollectionProperties read GetServerProperties;
{$ENDIF}
  end;
...
// *********************************************************************//
 TCustomXMLSchemaCollectionProperties = class(TPersistent)
  private
    FServer:    TCustomXMLSchemaCollection;
    function    GetDefaultInterface: _CustomXMLSchemaCollection;
    constructor Create(AServer: TCustomXMLSchemaCollection);
  protected
    function Get_Parent: IDispatch;
    function Get_Count: Integer;
    function Get_Item(Index: OleVariant): CustomXMLSchema;
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
  FProps := TCustomXMLSchemaCollectionProperties.Create(Self);
{$ENDIF}
end;
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
function TCustomXMLSchemaCollection.GetServerProperties: TCustomXMLSchemaCollectionProperties;
begin
  Result := FProps;
end;
{$ENDIF}
...
{$IFDEF LIVE_SERVER_AT_DESIGN_TIME}
constructor TCustomXMLSchemaCollectionProperties.Create(AServer: TCustomXMLSchemaCollection);
begin
  inherited Create;
  FServer := AServer;
end;

function TCustomXMLSchemaCollectionProperties.GetDefaultInterface: _CustomXMLSchemaCollection;
begin
  Result := FServer.DefaultInterface;
end;

function TCustomXMLSchemaCollectionProperties.Get_Parent: IDispatch;
begin
    Result := DefaultInterface.Parent;
end;

function TCustomXMLSchemaCollectionProperties.Get_Count: Integer;
begin
    Result := DefaultInterface.Count;
end;

function TCustomXMLSchemaCollectionProperties.Get_Item(Index: OleVariant): CustomXMLSchema;
begin
    Result := DefaultInterface.Item[Index];
end;

function TCustomXMLSchemaCollectionProperties.Get_NamespaceURI(Index: Integer): WideString;
begin
    Result := DefaultInterface.NamespaceURI[Index];
end;

function TCustomXMLSchemaCollectionProperties.Get__NewEnum: IUnknown;
begin
    Result := DefaultInterface._NewEnum;
end;
...
procedure Register;
begin
  RegisterComponents(dtlServerPage, [TCustomXMLSchemaCollection]);
end;
