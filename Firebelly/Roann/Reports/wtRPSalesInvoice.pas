...
type
  TfrmWTRPSalesInvoice = class(TForm)
    ...
    qrlblAccountManager: TgtQRLabel;
    ...
    qrlblNettAmountLabel: TgtQRLabel;
    QRLabel8: TgtQRLabel;
    QRShape7: TgtQRShape;
    QRShape8: TgtQRShape;
...
//  qrlblCompanyName.caption := qryCompanyAddress.fieldbyname('Company_Name').asstring;
  ...
  if trim(qryCompanyAddress.Fields[6].AsString) <> '' then
...
//      OurJobLbl.Caption := InvHeadSRC.Dataset.FieldByName('Reference').Asstring;
      ...
      qrlblReference.caption := GetSOReference(InvHeadSRC.Dataset.FieldByName('Reference').Asstring) ;
      lblOurOrderNo.caption := InvHeadSRC.Dataset.FieldByName('Reference').Asstring;
      qrlblAccountManager.caption := GetSOAccountManager(InvHeadSRC.Dataset.FieldByName('Reference').Asstring)
    end
  else
    begin
...
//      sInvType := GetSIType(InvHeadSRC.Dataset.FieldByName('Reference').Asstring);
      ...
      qrlblReference.caption := GetSOReference(JobReference);
      lblOurOrderNo.caption := JobReference;
      qrlblAccountManager.caption := GetSOAccountManager(JobReference);
