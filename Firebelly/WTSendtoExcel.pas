...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, ExtCtrls, OleCtnrs, Grids, DBGrids, AllCommon,
  Buttons, DB, QrCtrls;
...
procedure TfrmWTSendtoExcel.FormActivate(Sender: TObject);
begin
...
procedure TfrmWTSendtoExcel.ExportTableToExcel;
var
  icol, icolcount: integer;
  tmpTable: TFDTable;
  tmpDBField: TField;
...
begin
//  tmpQuery := TFDQuery.Create(self);
  tmpTable :=(ExportDBGrid.datasource.dataset as TFDTable);
  tmpDataSrc := (ExportDBGrid.datasource as TDataSource);

  prgbrExport.Max := tmpTable.RecordCount;
  tempFileName := getWinTempDir + ExportFormCaption + formatdatetime('yymmddhhmmss',now)+'.csv';
  ...
  // Set place in database to return back to
  SavePlace := tmpTable.GetBookmark;
  ...
  tmpTable.GotoBookmark(SavePlace);
...
procedure TfrmWTSendtoExcel.ExportToExcel;
var
