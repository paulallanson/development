...
type
  TfrmWTRSSalesOrder = class(TForm)
    btnPrint: TButton;
    Button4: TButton;
...
var
  frmWTRSSalesOrder: TfrmWTRSSalesOrder;

implementation

uses UITypes, allCommon, WTSrchCustomer, DateSelV5, WTRPSalesOrder;

{$R *.dfm}

procedure TfrmWTRSSalesOrder.Button4Click(Sender: TObject);
begin
  close;
end;

procedure TfrmWTRSSalesOrder.btnPrintClick(Sender: TObject);
begin
  RunReport(false);
end;

procedure TfrmWTRSSalesOrder.RunReport(const bPreview: boolean);
var
  iCount: integer;
begin
  frmwtRPSalesOrder := TfrmwtRPSalesOrder.create(self);
  try

//  Check if any records
    frmwtRPSalesOrder.customer := customer;
    frmwtRPSalesOrder.customerList.Clear;
    for iCount := 0 to pred(lstbxCustomersCode.Items.count) do
      begin
        frmwtRPSalesOrder.customerList.Add(lstbxCustomersCode.Items[icount]);
      end;
    if lstbxCustomersCode.Items.count > 0 then
      frmwtRPSalesOrder.customer := -1;

    frmwtRPSalesOrder.Rep := Rep;
    frmwtRPSalesOrder.datefrom := padatestr(edtDateFrom.Text);
    frmwtRPSalesOrder.dateto := padatestr(edtDateTo.Text);
    frmwtRPSalesOrder.Sortby := rdgrpSortBy.itemindex;

    frmwtRPSalesOrder.CustomerCategory := rdgrpCategory.itemindex;

    if frmwtRPSalesOrder.GetDetails = 0 then
      begin
        MessageDlg('No records to print', mtInformation, [mbOk], 0);
        exit;
      end;
    frmwtRPSalesOrder.qrlblCaption.Caption := frmwtRPSalesOrder.qrlblCaption.Caption;
    frmwtRPSalesOrder.qrlblDateRange.Caption := 'Sales Orders from ' + edtDateFrom.Text + ' to ' + edtDateTo.Text;
    frmwtRPSalesOrder.qrlblSelection.Caption := 'Reps: ' + cmbReps.Text + ' Customers: ' + lstbxCustomers.Items[0];
    frmwtRPSalesOrder.qrbGroupHeader.ForceNewPage := chkbxPageBreak.Checked;
    if bPreview then
      begin
          frmwtRPSalesOrder.qrpDetails.Preview;
      end
    else
      begin
          frmwtRPSalesOrder.qrpDetails.PrinterSetup;
          if frmwtRPSalesOrder.qrpDetails.tag = 0 then
            frmwtRPSalesOrder.qrpDetails.Print;
      end;
  finally
    frmwtRPSalesOrder.free;
  end;
end;

procedure TfrmWTRSSalesOrder.btnCustomerClick(Sender: TObject);
var
  iCount: integer;
begin
...
procedure TfrmWTRSSalesOrder.rdgrpCustomerClick(Sender: TObject);
begin
...
procedure TfrmWTRSSalesOrder.enableOK(Sender: TObject);
begin
...
procedure TfrmWTRSSalesOrder.btnDatefromClick(Sender: TObject);
var
  tempDate: string;
begin
...
procedure TfrmWTRSSalesOrder.edtDateFromExit(Sender: TObject);
var
  NewDate: TDateTime;
begin
...
function TfrmWTRSSalesOrder.InputDate(TempDate: TDateTime): TDateTime;
var
...
procedure TfrmWTRSSalesOrder.btnDateToClick(Sender: TObject);
var
  tempDate: string;
begin
...
procedure TfrmWTRSSalesOrder.FormCreate(Sender: TObject);
var
...
procedure TfrmWTRSSalesOrder.btnPreviewClick(Sender: TObject);
begin
  RunReport(true);
end;

procedure TfrmWTRSSalesOrder.btnExcelClick(Sender: TObject);
var
  iCount: integer;
  reccount: integer;
  tmpFileName: string;
...
//  Check if any records
    ...
    reccount := frmWTRPSalesOrder.GetDetails;
    if reccount = 0 then
      begin
    ...
    frmWTRPSalesOrder.ExportToFile(tmpFileName);
...
procedure TfrmWTRSSalesOrder.rdgrpRepsClick(Sender: TObject);
begin
...
procedure TfrmWTRSSalesOrder.cmbRepsClick(Sender: TObject);
begin
  qryReps.First;
...
procedure TfrmWTRSSalesOrder.LoadReps;
begin
  cmbReps.clear;
  with qryReps do
    begin
...
procedure TfrmWTRSSalesOrder.Delete1Click(Sender: TObject);
var
  iCount: integer;
begin
...
procedure TfrmWTRSSalesOrder.pmnCustomersPopup(Sender: TObject);
begin
