...
type
  ...
  private
    ...
    function PriceBySlab(tmpQuote: integer): boolean;
  public
    SalesOrder: integer;
    bDetailed: boolean;
    bPreview: boolean;
    bPriceBySlab: boolean;
    bPrintLogo: boolean;
...
//  qrlblTotalPrice.caption := formatfloat('0.00',totalPrice);
  ...
  bPriceBySlab := dtmdlWorktops.UseSlabContractQuoting and PriceBySlab(qrySOLine.FieldByName('Quote').Asinteger);
...
function TfrmwtRPSOrder.PriceBySlab(tmpQuote: integer): boolean;
begin
  result := false;
  with qryGetQuote do
    begin
      ...
      open;
      result := (fieldbyname('Contract_Quote_ID').asinteger <> 0);
    end;
end;
