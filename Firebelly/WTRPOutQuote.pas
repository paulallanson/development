...
var
  frmwtRPOutQuote: TfrmwtRPOutQuote;

implementation

uses wtDataModule, wtRSOutQuote, AllCommon;

{$R *.dfm}

procedure TfrmwtRPOutQuote.qrpDetailsBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  CustomerTotal := 0.00;
  ReportTotal := 0.00;
...
procedure TfrmwtRPOutQuote.qrsbDetailsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
...
begin
  ...
  qrlblEvent.caption := paDatestr(qryQuotes.fieldbyname('Last_Event').asdatetime);
...
procedure TfrmwtRPOutQuote.qrbCustomerFooterAfterPrint(
  Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  CustomerTotal := 0.00;
end;

procedure TfrmwtRPOutQuote.qrbCustomerFooterBeforePrint(
...
function TfrmwtRPOutQuote.GetDetails: integer;
  { Local function }
...
procedure TfrmwtRPOutQuote.QRBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  ReportTotal := 0.00;
end;

procedure TfrmwtRPOutQuote.ExporttoFile(filename: string);
var
  TempName: string;
  tempStr: string;
begin
...
procedure TfrmwtRPOutQuote.qrsbDetailsAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
var
  tempstr: string;
begin
  ...
  begin
    ...
    writeln(self.exportFile, tempStr);
    frmWTRSOutQuote.prgbrExport.StepIt;
  end;
end;

procedure TfrmwtRPOutQuote.qrgCustomerHeadBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if qryQuotes.fieldbyname('Is_Retail_Customer').asstring = 'Y' then
...
procedure TfrmwtRPOutQuote.SetCustomerCategory(const Value: integer);
begin
