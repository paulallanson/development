...
type
  TfrmWTRSQuoteExpiry = class(TForm)
    btnPrint: TButton;
    Button4: TButton;
...
var
  frmWTRSQuoteExpiry: TfrmWTRSQuoteExpiry;

implementation

uses UITypes, WTRPQuoteExpiry, allCommon, WTSrchCustomer, DateSelV5;

{$R *.dfm}

procedure TfrmWTRSQuoteExpiry.Button4Click(Sender: TObject);
begin
  close;
end;

procedure TfrmWTRSQuoteExpiry.btnPrintClick(Sender: TObject);
begin
  RunReport(false);
end;

procedure TfrmWTRSQuoteExpiry.RunReport(const bPreview: boolean);
begin
  frmwtRPQuoteExpiry := TfrmwtRPQuoteExpiry.create(self);
  try

//  Check if any records
    frmwtRPQuoteExpiry.customer := customer;
    frmwtRPQuoteExpiry.Operator := Operator;
    frmwtRPQuoteExpiry.datefrom := padatestr(edtDateFrom.Text);
    frmwtRPQuoteExpiry.dateto := padatestr(edtDateTo.Text);

    frmwtRPQuoteExpiry.CustomerCategory := rdgrpCategory.itemindex;

    if frmwtRPQuoteExpiry.GetDetails = 0 then
      begin
        MessageDlg('No records to print', mtInformation, [mbOk], 0);
        exit;
      end;
    frmwtRPQuoteExpiry.qrlblCaption.Caption := frmwtRPQuoteExpiry.qrlblCaption.Caption + ' - run date: ' + padatestr(date);
    frmwtRPQuoteExpiry.qrlblDateRange.Caption := 'Raised between ' + edtDateFrom.Text + ' to ' + edtDateTo.Text;
    frmwtRPQuoteExpiry.qrgCustomerHead.ForceNewPage := chkbxPageBreak.checked;

    if bPreview then
      begin
          frmwtRPQuoteExpiry.qrpDetails.Preview;
      end
    else
      begin
          frmwtRPQuoteExpiry.qrpDetails.PrinterSetup;
          if frmwtRPQuoteExpiry.qrpDetails.tag = 0 then
            frmwtRPQuoteExpiry.qrpDetails.Print;
      end;
  finally
    frmwtRPQuoteExpiry.free;
  end;
end;

procedure TfrmWTRSQuoteExpiry.btnCustomerClick(Sender: TObject);
begin
...
procedure TfrmWTRSQuoteExpiry.rdgrpCustomerClick(Sender: TObject);
begin
...
procedure TfrmWTRSQuoteExpiry.enableOK(Sender: TObject);
begin
...
procedure TfrmWTRSQuoteExpiry.btnDatefromClick(Sender: TObject);
var
  tempDate: string;
begin
...
procedure TfrmWTRSQuoteExpiry.edtDateFromExit(Sender: TObject);
var
  NewDate: TDateTime;
begin
...
function TfrmWTRSQuoteExpiry.InputDate(TempDate: TDateTime): TDateTime;
var
...
procedure TfrmWTRSQuoteExpiry.btnDateToClick(Sender: TObject);
var
  tempDate: string;
begin
...
procedure TfrmWTRSQuoteExpiry.FormCreate(Sender: TObject);
var
  DateFrom: TDateTime;
begin
  customer := 0;
...
procedure TfrmWTRSQuoteExpiry.btnExcelClick(Sender: TObject);
var
  reccount: integer;
  tmpFileName: string;
begin
  ...
  try
    ...
    reccount := frmwtRPQuoteExpiry.GetDetails;
    if reccount = 0 then
      begin
        MessageDlg('No records to print', mtInformation, [mbOk], 0);
        exit;
    ...
    frmwtRPQuoteExpiry.ExportToFile(tmpFileName);
...
procedure TfrmWTRSQuoteExpiry.btnPreviewClick(Sender: TObject);
begin
  RunReport(true);
end;

procedure TfrmWTRSQuoteExpiry.rdgrpContactsClick(Sender: TObject);
begin
...
procedure TfrmWTRSQuoteExpiry.cmbContactsClick(Sender: TObject);
begin
  qryContacts.First;
...
procedure TfrmWTRSQuoteExpiry.LoadOfficeContacts;
begin
  cmbContacts.clear;
  with qryContacts do
    begin
