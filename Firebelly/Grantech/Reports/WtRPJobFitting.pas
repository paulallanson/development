...
type
  TfrmwtRPJobFitting = class(TForm)
    ...
    qrrchTextRiskNotes: TQRRichText;
...
begin
  ...
  with qryJobUpstand do
    begin
      ...
      while eof <> true do
        begin
          if (pos('Upstand',fieldbyname('Use_Description').AsString) = 1) or (fieldbyname('Use_Description').AsString = '') then
            qrlblUpstand.caption := 'Yes'
          else
          if (pos('Window Cill',fieldbyname('Use_Description').AsString) = 1) then
            qrlblWindowCill.caption := 'Yes'
          else
          if (pos('Splashback',fieldbyname('Use_Description').AsString) = 1) then
