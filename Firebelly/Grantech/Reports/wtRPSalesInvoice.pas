...
type
  TfrmWTRPSalesInvoice = class(TForm)
    ...
    QRLabel8: TgtQRLabel;
    QRLabel9: TgtQRLabel;
    qrlblVatAmountLabel: TgtQRLabel;
    GoodsLbl: TgtQRLabel;
    qrlblVatReg: TgtQRLabel;
    qrlblTotalNettLabel: TgtQRLabel;
    qrlblTotalVatLabel: TgtQRLabel;
  ...
  private
    ...
    function GetSOAccountManager(tempCode: string): string;
    function GetSOReference(tempCode: string): string;
  public
    bInvoice: boolean;
    bDetailed: boolean;
    bUpdate: boolean;
...
//  tempAddress := qryCompanyAddress.fieldbyname('Company_Name').AsString;
  tempAddress := qryCompanyAddress.fieldbyname('Street').AsString;
...
//    tempAddress := tempAddress + #13#10 + qryCompanyAddress.Fields[icount].AsString;
  ...
  if qryCompanyAddress.Fields[5].AsString <> '' then
    begin
      tempAddress := tempAddress + #13#10;
      tempAddress := tempAddress + 'Tel: ' + qryCompanyAddress.Fields[5].AsString;
    end;

  if qryCompanyAddress.Fields[6].AsString <> '' then
    begin
      tempAddress := tempAddress + #13#10;
      tempAddress := tempAddress + 'Fax: ' + qryCompanyAddress.Fields[6].AsString;
    end;

  if qryCompanyAddress.fieldbyname('Email_Address').AsString <> '' then
    begin
      tempAddress := tempAddress + #13#10;
      tempAddress := tempAddress + 'Email: ' + qryCompanyAddress.fieldbyname('Email_Address').AsString;
    end;

  if qryCompanyAddress.fieldbyname('Web_Address').AsString <> '' then
    begin
      tempAddress := tempAddress + #13#10;
      tempAddress := tempAddress + 'Web: ' + qryCompanyAddress.fieldbyname('Web_Address').AsString;
    end;
...
begin
  ...
  qrlblTotalNettLabel.enabled := not bReverseCharge;
  qrlblTotalVatLabel.enabled := not bReverseCharge;
  VatValuelbl.enabled := not bReverseCharge;
  GoodsValuelbl.enabled := not bReverseCharge;
...
function TfrmWTRPSalesInvoice.GetSOReference(tempCode: string): string;
begin
  result := '';
  with qrySOHead do
    begin
...
function TfrmWTRPSalesInvoice.GetSOAccountManager(tempCode: string): string;
begin
  result := '';
  with qrySOHead do
    begin
      ...
      parambyname('Sales_order').asinteger := strtoint(trim(tempcode));
      open;
      result := qrySOHead.fieldbyname('Account_Manager_Name').asstring;
    end;
end;
...
procedure TfrmWTRPSalesInvoice.qrsdQEdgesBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
...
procedure TfrmWTRPSalesInvoice.qrsdQCutOutsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  rUnitPrice, rTotalPrice: real;
begin
