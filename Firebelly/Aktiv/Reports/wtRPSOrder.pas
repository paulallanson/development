...
type
  TfrmwtRPSOrder = class(TForm)
    ...
    qrlblBalanceCaption: TgtQRLabel;
...
*)
  ...
  if PrintType = 'I' then
    rBalance := rGrosstotal - qryReport.FieldByName('Deposit_amount').Asfloat - rToPay
  else
...
begin
  ...
  if FPrintType = 'I' then
    begin
      ...
      qrlblSOCaption.Caption := 'PROFORMA';
      qrlblDepositToPay.Caption := 'Interim Payment'
    end
  else
    begin
      qrlblBalanceCaption.enabled := false;
