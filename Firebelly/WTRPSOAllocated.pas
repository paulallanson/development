...
type
  TfrmWTRPSOAllocated = class(TForm)
    ...
    qrlblAllocatedSlabDescription: TQRLabel;
...
var
  frmWTRPSOAllocated: TfrmWTRPSOAllocated;

implementation

uses wtDataModule, WTRSSOAllocated, AllCommon;

{$R *.dfm}

procedure TfrmWTRPSOAllocated.qrpDetailsBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  case sortby of
    0:begin
...
procedure TfrmWTRPSOAllocated.qrsbDetailsBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  iCount: integer;
begin
...
//      qrsbDetails.Font.Style := [fsitalic, fsbold];
  ...
  if qrySalesOrders.FieldByName('Allocated_Slab_Length').Asstring = '' then
    qrlblAllocatedSlabDescription.caption := ''
  else
    qrlblAllocatedSlabDescription.caption := qrySalesOrders.FieldByName('Allocated_Slab_Length').Asstring + ' x ' + qrySalesOrders.FieldByName('Allocated_Slab_Depth').Asstring;
...
function TfrmWTRPSOAllocated.GetDetails: integer;
var
  sDateTo: string;
  iCount: integer;
  { Local function }
...
begin
  ...
  {We only want allocated order lines and slabs}
...
procedure TfrmWTRPSOAllocated.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TfrmWTRPSOAllocated.QRBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  ReportTotal := 0.00;
end;

procedure TfrmWTRPSOAllocated.ExporttoFile(filename: string);
var
  tempStr: string;
begin
  self.exporting := true;
...
procedure TfrmWTRPSOAllocated.qrsbDetailsAfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
var
  tempstr: string;
begin
  ...
  begin
    ...
    //Slab Allocated
    tempStr := tempStr + ',"' + qrySalesOrders.fieldbyname('Slab_Allocated').asstring + '"';

    //Allocated Slab Size
    tempStr := tempStr + ',"' + qrlblAllocatedSlabDescription.caption + '"';

    //Allocated Stock Code
    tempStr := tempStr + ',"' + qrySalesOrders.fieldbyname('Allocated_Stock_Code').asstring + '"';
    ...
    writeln(self.exportFile, tempStr);
    frmWTRSSOAllocated.prgbrExport.StepIt;
  end;
end;

procedure TfrmWTRPSOAllocated.SetCustomerCategory(const Value: integer);
begin
  FCustomerCategory := Value;
end;

procedure TfrmWTRPSOAllocated.FormCreate(Sender: TObject);
begin
  customerList := TStringList.Create;
end;

procedure TfrmWTRPSOAllocated.FormDestroy(Sender: TObject);
begin
  customerList.Free;
end;

procedure TfrmWTRPSOAllocated.SetSelectBy(const Value: integer);
begin
  FSelectBy := Value;
end;
